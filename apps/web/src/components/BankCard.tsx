...
export default function BankCard({ bank, onUpdate, onDelete }: BankCardProps) {
  ...
  if (isRenaming) {
    return (
      <div className="rounded-lg border border-gray-300 bg-white p-4 shadow-sm ring-1 ring-gray-300">
        <form onSubmit={handleRenameSubmit} className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-brand-blue focus:outline-none focus:ring-1 focus:ring-brand-blue"
            />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className="rounded bg-brand-blue px-3 py-1 text-xs font-medium text-white hover:opacity-90 disabled:opacity-50"
            >
  ...
  // --- View mode ---
  return (
    <div className={`flex flex-col rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-opacity ${isPending ? 'opacity-50' : ''}`}>
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={isPending}
            className="rounded px-2 py-1 text-xs font-medium text-brand-red hover:bg-red-50 disabled:opacity-50"
